class TransactionRecordsController < ApplicationController
  ...
  def download_pdf
    from = params[:record][:date_from]
    to = params[:record][:date_to]
    @record = TransactionRecord.where('user_id=?', current_user.id)
    respond_to do |format|
      format.pdf do
        pdf = Prawn::Document.new
        table_data = Array.new
        table_data << ["Process Name", "Process category", "Amount", "Date"]
        @record.each do |p|
          if p.process.date >= Date.parse(from) && p.process.date <= Date.parse(to)
            if p.process_type == 'Income'
              category = p.process.income_category.income_source
            elsif p.process_type == 'Expense'
              category = p.process.expense_category.expense_source
            end
